...
const palette = ["#2df4c0", "#32b8ff", "#ffd166", "#ff5578", "#7c8cff"];
...
export function PowerPage() {
  ...
  return <><PageHead eyebrow="动力环境 · 2,142 测点" title="动力环境" description="供配电、制冷与环境参数一体化监测" actions={<div className="segmented">{["全部", "动力", "环境"].map(x => <button key={x} className={filter === x ? "active" : ""} onClick={() => setFilter(x)}>{x}</button>)}</div>} />
    <div className="power-layout"><aside className="system-tree glass-card"><h2>设备分组</h2>{[[Zap,"供配电系统","186"],[BatteryCharging,"UPS系统","24"],[Snowflake,"精密空调","48"],[Thermometer,"温湿度","326"],[Droplets,"漏水检测","64"]].map(([Icon,n,c]) => { const C = Icon as typeof Zap; return <div className="tree-row" key={String(n)}><C /><span>{n}</span><b>{c}</b><ChevronRight /></div>})}</aside><div className="power-main"><div className="mini-stats"><Stat label="总有功功率" value="1.84" unit="MW" icon={<CloudLightning />} /><Stat label="平均温度" value="22.4" unit="°C" icon={<Thermometer />} tone="blue" /><Stat label="制冷负载" value="73" unit="%" icon={<Snowflake />} tone="blue" /></div><Glass title="动力负载与能耗趋势" subtitle="今日分时数据" icon={<Activity />}><Chart type="energy" className="h-72" /></Glass></div></div>
...
export function SecurityPage() {
  return <><PageHead eyebrow="安防态势 · 布防中" title="安防消防" description="视频、门禁、消防与周界联动监管" />
    <div className="security-strip">{[[Camera,"摄像机","126 / 128"],[DoorOpen,"门禁控制器","32 / 32"],[Flame,"消防回路","18 / 18"],[ShieldCheck,"布防区域","12 / 12"]].map(([Icon,n,v],i)=>{const C=Icon as typeof Camera; return <div className="security-kpi" key={String(n)}><C/><span>{n}<b>{v}</b></span><StatusPill tone={i===0?"warn":"ok"}>{i===0?"2 离线":"正常"}</StatusPill></div>})}</div>
