...
function Chart({ type, className = "h-64" }: { type: "energy" | "pie" | "bar" | "radar" | "temp" | "alarm" | "pue"; className?: string }) {
  const option = useMemo((): EChartsOption => {
    ...
    const configs: { names: [string, string]; a: number[]; b: number[] } = type === "temp" ? { names: ["温度", "湿度"], a: [21.8, 21.6, 22.4, 23.1, 22.7, 22.2, 21.9], b: [48, 49, 47, 45, 46, 48, 49] } : type === "pue" ? { names: ["PUE", "目标线"], a: [1.38, 1.35, 1.33, 1.31, 1.29, 1.28, 1.30], b: [1.32, 1.32, 1.32, 1.32, 1.32, 1.32, 1.32] } : { names: ["总能耗", "IT负载"], a: [610, 580, 720, 840, 790, 730, 650], b: [420, 408, 505, 578, 548, 510, 452] };
